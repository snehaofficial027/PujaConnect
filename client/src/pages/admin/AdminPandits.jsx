...
const AdminPandits = () => {
  ...
  const fetchPandits = async () => {
    try {
      ...
      if (res.data.success || res.data) {
        setPandits(res.data.pandits || res.data || []);
      }
    } catch (err) {
  ...
  const deletePandit = async (id) => {
    if (!window.confirm("Are you sure you want to delete this pandit?")) return;
    try {
      const token = localStorage.getItem("adminToken");
      await API.delete(`/api/admin/pandits/${id}`, {
  ...
  const approved = pandits.filter((p) => p.approved).length;
  const pending = pandits.filter((p) => !p.approved).length;

  const adminEarnings = pandits.reduce(
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-5">
          <div className="bg-white rounded-3xl p-6 shadow">
            <p className="text-orange-600 font-medium">Total Earnings</p>
            ...
            >
              ...
              <div className="relative h-56 bg-gray-100">
                <img
                  ...
                  className="w-full h-full object-cover object-top bg-gray-100 p-2"
                />
              ...
              <div className="p-5">
                ...
                <div className="mt-5 space-y-3 text-sm">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="truncate">{pandit.email}</span>
                  </div>
      ...
      {showModal && selectedPandit && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden relative">
            <button
              onClick={closeModal}
              className="absolute top-5 right-5 w-10 h-10 rounded-full bg-red-500 hover:bg-red-600 text-white text-xl flex items-center justify-center"
            >
              ✕
            </button>
              ...
              <div className="md:col-span-2 p-8">
                ...
                <div className="grid md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <h3 className="font-semibold">{selectedPandit.email}</h3>
                  </div>

                  <div>
                    <p className="text-gray-500 text-sm">Phone</p>
                    <h3 className="font-semibold">{selectedPandit.phone}</h3>
                  </div>

                  <div>
                    <p className="text-gray-500 text-sm">City</p>
                    <h3 className="font-semibold">{selectedPandit.city}</h3>
                  </div>

                  <div>
                    <p className="text-gray-500 text-sm">Language</p>
                    <h3 className="font-semibold">
                      {selectedPandit.language || selectedPandit.languages}
                    </h3>
                  </div>
                  ...
                  <div>
                    ...
                    <h3 className="font-semibold">
                      ⭐ {selectedPandit.rating || "5.0"} ({selectedPandit.reviews || 0})
                    </h3>
                  </div>
                ...
                <div className="mt-8">
                  ...
                  <p className="text-gray-600 leading-7">
                    {selectedPandit.bio || selectedPandit.about || "No bio available"}
                  </p>
                </div>
